...
def build_pgn_from_json(game_data):
    ...
    white_name = (
        game_data.get("players", {}).get("white", {}).get("name", "Unknown")
    )
    black_name = (
        game_data.get("players", {}).get("black", {}).get("name", "Unknown")
    )
...
def analyze_pending_games(limit=None):
    ...
    try:
        ...
        with (
            AchievementAnalyzer(low_depth=1, high_depth=16) as analyzer,
            ...
            with conn.cursor() as cur:
                ...
                for game_id, game_data_raw in tqdm(
                    ...
                    try:
                        ...
                        annotated_pgn, local_evals = analyzer.analyze_game(
                            pgn_string
                        )
                        ...
                        if annotated_pgn:
                            ...
                            logger.warning(
                                "  ⚠️ Game %s skipped (Analyzer returned no data)",
                                game_id,
                            )

                    except Exception as game_err:
                        logger.error(
                            "  ❌ Failed game %s: %s", game_id, game_err
                        )
