...
class StockfishEvaluator:
    ...
    def evaluate_game(
        ...
        amended_pgn = game.accept(chess.pgn.StringExporter(columns=None, comments=True, variations=True))
    ...
    def _analyze_position(self, board: chess.Board, played_move: chess.Move, ply: int) -> MoveAnalysis:
        ...
        high_res = self.engine.analyse(board, chess.engine.Limit(depth=self.high_depth), multipv=2)
        ...
        if not played_move_info:
            played_move_info = self.engine.analyse(
                board, chess.engine.Limit(depth=self.high_depth), root_moves=[played_move]
            )
        ...
        return {
            ...
            "high_depth_eval": self._parse_score(played_move_info.get("score"), board.turn),
            "high_top_moves": [
                {"move": board.san(info["pv"][0]), "eval": self._parse_score(info["score"], board.turn)}
    ...
    def _run_mate_grace_period(self, board: chess.Board, terminal_node: chess.pgn.GameNode, max_plies: int) -> None:
    ...
    @staticmethod
    def _parse_score(score_obj: Optional[chess.engine.PovScore], turn: chess.Color) -> EvalScore:
        if not score_obj:
