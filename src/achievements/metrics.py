...
def _is_clean_capture_quiescent(
    ...
    def get_net_balance(b):
        return len(b.pieces(captured_type, my_color)) - len(b.pieces(captured_type, not my_color))
...
class GameMetrics:
    ...
    def __init__(
        ...
        if self.is_win:
            ...
            if self.speed == "ultrabullet": self.is_ultrabullet_win = True
            elif self.speed == "bullet": self.is_bullet_win = True
            elif self.speed == "blitz": self.is_blitz_win = True
            elif self.speed == "rapid": self.is_rapid_win = True
            elif self.speed == "classical": self.is_classical_win = True
    ...
    def _aggregate_metrics(
        ...
        for ply, move_san in enumerate(san_moves, start=1):
            ...
            if (ply - 1) < len(annotated_plies):
                ...
                if is_my_turn and board.is_capture(move) and cls not in ("mistake", "blunder"):
                    ...
                    if _is_clean_capture_quiescent(san_moves, ply, self.my_color, captured_piece):
                        if captured_piece == chess.PAWN: self.clean_pawns_count += 1
                        elif captured_piece == chess.KNIGHT: self.clean_knights_count += 1
                        elif captured_piece == chess.BISHOP: self.clean_bishops_count += 1
                        elif captured_piece == chess.ROOK: self.clean_rooks_count += 1
                        elif captured_piece == chess.QUEEN: self.clean_queens_count += 1
        ...
        if self.is_win:
            if self.total_plies <= 20: self.win_phase = "opening"
            elif self.total_plies <= 60: self.win_phase = "midgame"
            else: self.win_phase = "endgame"
