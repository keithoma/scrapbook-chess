...
def is_clean_capture_quiescent(
    ...
    def get_net_material(b: chess.Board) -> int:
        ...
        opp_mat = sum(
            len(b.pieces(pt, not my_color)) * val for pt, val in values.items()
        )
    ...
    while chain_start_ply > 1:
        ...
        try:
            ...
            # If the preceding move was also a capture, check, or promotion, the
            # sequence started earlier
            if (
                prev_board.is_capture(move)
                or prev_board.gives_check(move)
                or move.promotion
            ):
                chain_start_ply -= 1
            else:
    ...
    # If the game ends before 10 plies are up, but we maintained the advantage, it
    # counts
    return True
